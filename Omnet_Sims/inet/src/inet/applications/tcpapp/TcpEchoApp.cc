...
{
    EV << "S&S: EchoApp sending down." << endl;
    if (msg->isPacket()) {
        Packet *pk = static_cast<Packet *>(msg);
        bytesSent += pk->getByteLength();
        emit(packetSentSignal, pk);
        EV << "S&S: packet arrival time: " << pk->getArrivalTime() << endl;
        EV << "S&S: packet arrival gate id: " << pk->getArrivalGateId() << endl;
        EV << "S&S: packet id: " << pk->getId() << endl;
        EV << "S&S: packet sender gate id: " << pk->getSenderGateId() << endl;
        EV << "S&S: packet source proc id: " << pk->getSrcProcId() << endl;
        EV << "S&S: packet total message count: " << pk->getTotalMessageCount() << endl;
    }

    msg->addTagIfAbsent<DispatchProtocolReq>()->setProtocol(&Protocol::tcp);
    msg->getTag<SocketReq>();

//    EV << "S&S: packet socketReq tag is: " << msg->getTag<SocketReq>() << endl;
    send(msg, "socketOut");
}
